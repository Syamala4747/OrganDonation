...
const Login = () => {
  ...
  const mobileStyles = `
    @media (max-width: 700px) {
      ...
      .mobile-auth-left, .mobile-auth-right {
      ...
      .mobile-auth-form input, .mobile-auth-form select {
        font-size: 1rem !important;
        padding: 0.7rem 1rem !important;
      }
      .mobile-auth-grid {
  ...
  // Remove username state for login
  ...
  const handleRegister = async (e) => {
    ...
    setError('');
    try {
      const res = await axios.post('/api/auth/donor-signup', { username, email, password, category });
  ...
  <div className="mobile-auth-card" style={{width:'100%', maxWidth:'340px', marginBottom:'1.2rem'}}>
          ...
          <div style={{display:'flex', background:'#f3f4f6', borderRadius:'1.2rem', overflow:'hidden', marginBottom:'1rem', width:'100%'}}>
            <button style={{flex:1, padding:'0.5rem 0', fontWeight:'bold', fontSize:'1rem', background:tab==='login'?'#fff':'#f3f4f6', border:'none', outline:'none', cursor:'pointer', color:'#222'}} onClick={()=>setTab('login')}>Login</button>
            <button style={{flex:1, padding:'0.5rem 0', fontWeight:'bold', fontSize:'1rem', background:tab==='register'?'#fff':'#f3f4f6', border:'none', outline:'none', cursor:'pointer', color:'#222'}} onClick={()=>setTab('register')}>Register</button>
          </div>
          {tab==='login' && (
          ...
          {tab==='register' && (
            <div style={{background:'#fff', borderRadius:'1rem', boxShadow:'0 2px 12px rgba(0,0,0,0.07)', padding:'1.2rem 1rem', width:'100%', margin:'0 auto'}}>
              ...
              <form className="mobile-auth-form" onSubmit={handleRegister}>
                ...
                <label className="mobile-auth-label" style={{fontWeight:'500', fontSize:'0.85rem', marginBottom:'0.45rem', display:'block'}}>Category</label>
                <select value={category} onChange={e => setCategory(e.target.value)} required style={{width:'100%', padding:'0.8rem 1.2rem', borderRadius:'1.2rem', border:'1px solid #e5e7eb', background:'#f3f4f6', fontSize:'1rem', marginBottom:'0.8rem', boxSizing:'border-box'}}>
